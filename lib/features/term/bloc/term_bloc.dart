...
class TermBloc extends Bloc<TermEvent, TermState> {
  ...
  TermBloc({this.repository = const TermRepository()}) : super(const TermInitial()) {
  ...
  Future<void> _filePressed(TermChooseFiles event, Emitter<TermState> emit) async =>
  ...
  Future<void> _chooseRangeDate(TermChooseRangeDate event, Emitter<TermState> emit) async {
    print(
      '${("-" * 100).toString()}\n'
      '${event.start}\n'
      '${event.end}\n'
